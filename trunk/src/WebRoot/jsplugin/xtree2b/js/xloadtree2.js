...
webFXTreeConfig.loadingIcon = "jsplugin/xtree2b/images/loading.gif";
...
WebFXLoadTree.loadXmlDocument = function (jsNode)
{
	...
	if(window.XMLHttpRequest){
		jsNode._xmlHttp=new XMLHttpRequest(); 
		if(jsNode._xmlHttp.overrideMimeType){
			jsNode._xmlHttp.overrideMimeType("text/xml"); 
		} 
	}else if(window.ActiveXObject){
		try{ 
			jsNode._xmlHttp=new ActiveXObject("Msxml2.XMLHTTP"); 
		}catch(e){ 
			try{ 
				jsNode._xmlHttp=new ActiveXObject("Microsoft.XMLHTTP"); 
			}catch(e){ 
			} 
		} 
	} 
	...
	jsNode._xmlHttp.open("POST", jsNode.src, true);	// async
	jsNode._xmlHttp.setRequestHeader("Content-Type", "text/xml;charset=UTF-8");
	jsNode._xmlHttp.setRequestHeader("cache-control","no-cache");
	jsNode._xmlHttp.setRequestHeader("If-Modified-Since","0");
