...
if ( !this.ParentWindow ){
	...
	arr.push("            </div></td></tr>");
	arr.push("      </table><a href='#;' onfocus='$SET(\"_forTab_"+this.ID+"\").focus();'></a></td>");
	...
	var div = pw.$SET("_DialogDiv_"+this.ID);
	if(!div){
	...
	if(this.innerElementId){
		var innerElement=$SET(this.innerElementId);
		...
		if(isIE){
			...
			pw.$SET("_Covering_"+this.ID).parentNode.appendChild(fragment)
		}else{
			pw.$SET("_Covering_"+this.ID).parentNode.appendChild(innerElement)
		}
	}
	pw.$SET("_DialogDiv_"+this.ID).DialogInstance = this;
	if(this.URL)
		pw.$SET("_DialogFrame_"+this.ID).DialogInstance = this;
	pw.Drag.init(pw.$SET("_draghandle_"+this.ID),pw.$SET("_DialogDiv_"+this.ID));//注册拖拽方法
	if(!isIE){
		pw.$SET("_DialogDiv_"+this.ID).dialogId=this.ID;
		pw.$SET("_DialogDiv_"+this.ID).onDragStart = function(){$SET("_Covering_"+this.dialogId).style.display=""}
		pw.$SET("_DialogDiv_"+this.ID).onDragEnd = function(){$SET("_Covering_"+this.dialogId).style.display="none"}
	}

	this.OKButton = pw.$SET("_ButtonOK_"+this.ID);
	this.CancelButton = pw.$SET("_ButtonCancel_"+this.ID);

	//显示标题图片
	if(this.ShowMessageRow){
		$E.show(pw.$SET("_MessageRow_"+this.ID));
		if(this.MessageTitle){
			pw.$SET("_MessageTitle_"+this.ID).innerHTML = this.MessageTitle;
		}
		if(this.Message){
			pw.$SET("_Message_"+this.ID).innerHTML = this.Message;
		}
	}

	//显示按钮栏
	if(!this.ShowButtonRow){
		pw.$SET("_ButtonRow_"+this.ID).hide();
	}
	if(this.CancelEvent){
...
Dialog.prototype.close = function(){
	if(this.innerElementId){
		var innerElement=$E.getTopLevelWindow().$SET(this.innerElementId);
	...
	if(this.WindowFlag){
		...
		if(this.AlertFlag){
			$E.hide(pw.$SET("_AlertBGDiv"));
		}
		if(!flag&&!this.AlertFlag){//此处是为处理弹出窗口被关闭后iframe立即被重定向时背景层不消失的问题
			pw.eval('window._OpacityFunc = function(){var w = $E.getTopLevelWindow();$E.hide(w.$SET("_DialogBGDiv"));}');
			pw._OpacityFunc();
		}
...
Dialog.prototype.addButton = function(id,txt,func){
	...
	var pw = $E.getTopLevelWindow();
	pw.$SET("_DialogButtons_"+this.ID).$T("input")[0].getParent("a").insertAdjacentHTML("beforeBegin",html);
	pw.$SET("_Button_"+this.ID+"_"+id).onclick = func;
}
...
Dialog.alert = function(msg,func,w,h){
	...
	diag.show();
	pw.$SET("_AlertBGDiv").style.display="";
	$E.hide(pw.$SET("_ButtonOK_"+diag.ID));
	var win = pw.$SET("_DialogFrame_"+diag.ID).contentWindow;
	...
	pw.$SET("_DialogButtons_"+diag.ID).style.textAlign = "center";
}
...
Dialog.confirm = function(msg,func1,func2,w,h){
	...
	diag.show();
	pw.$SET("_AlertBGDiv").style.dispaly="";
	var win = pw.$SET("_DialogFrame_"+diag.ID).contentWindow;
	var doc = win.document;
	doc.open();
	doc.write("<body oncontextmenu='return false;'></body>") ;
	...
	if(d){
		try{
			...
			if(!this.AlertFlag){
				...
				if(!flag){
					$E.getTopLevelWindow().$SET("_DialogBGDiv").style.opacity="0.4";
					$E.getTopLevelWindow().$SET("_DialogBGDiv").style.filter="alpha(opacity=40)";
				}
			}
			if(d.AlertFlag){
				$E.show($E.getTopLevelWindow().$SET("_AlertBGDiv"));
			}
...
Dialog.setPosition=function(){
	...
	{
		pw.$SET("_DialogDiv_"+DialogArr[i]).DialogInstance.setPosition();
	}
}
Dialog.prototype.setPosition=function(){
	...
	pw.$SET(this.bgdivID).style.height= sh + "px";
}
